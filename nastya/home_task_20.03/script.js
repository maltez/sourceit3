...
class Enemy extends Tank {
	...
	move() {
		...
		//Algorithm of tank movement
		function back() {
			self.left = parseInt(CurrentPos.left);
			self.top = parseInt(CurrentPos.top);
			if (self.top > 0 && self.left >= 0) {
				self.enemyObj.style.left = self.left - 5 + "px";
				self.enemyObj.style.top = self.top - 5 + "px";
			} else {
				setInterval(forward, 100);
			}
		}
		...
		function forward() {
			// determine the current position of tank;
			self.left = parseInt(CurrentPos.left);
			self.top = parseInt(CurrentPos.top);
			if (self.top < 300 && self.left < 400) {
				self.enemyObj.style.left = self.left + 5 + "px";
				self.enemyObj.style.top = self.top + 5 + "px";
			} else {
